...
class CBuscar extends CI_Controller {
	...
	private $id_cat_profesion="-1";
	private $id_cat_estado="-1";
	private $filtrar="";
	...
	public function Profesiones(){
		$resultado['profesiones'] = $this->MBuscar->ListadoProfesiones();						
		echo json_encode($resultado);
	}

	public function Estados(){
		$resultado['estados'] = $this->MBuscar->ListadoEstados();				
		echo json_encode($resultado);
	}
	...
	public function loadRecord()
	{
		...
		$config = array();
        $config["base_url"] = base_url() . "CBuscar";
        $config["total_rows"] = $this->MBuscar->get_count(); 
        $config["per_page"] = 20;
		...
		$pagination_data['profesionistas'] = $this->MBuscar->ListadoProfesionales($config["per_page"], $page);	
	...
	public function Citas(){
		$resultado['citas'] = $this->MBuscar->ConsultaCitasProfesional();						
		echo json_encode($resultado);
	}
}
